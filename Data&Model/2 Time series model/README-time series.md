...
### 4.1 Bit-Exact Reproduction (EGARCH)

While the eight non-EGARCH models (AR, ARMA, ARCH, GARCH) reproduce exactly on any
BLAS backend, the two EGARCH models require a **specific BLAS backend** for bit-exact
reproduction. This is because EGARCH with Student-t errors and SLSQP optimization on
small rolling windows has a multi-modal loss surface that is highly sensitive to the
last-bit floating-point reduction order of the BLAS/LAPACK library. A single-bit
difference in gradient evaluation can push the optimizer into a different local basin,
producing RMSE values that differ by orders of magnitude.

The paper's original results were produced using **Intel MKL 2021.4.0 on Windows**.
Reproducing the exact EGARCH values therefore requires the same BLAS binary on the
same operating system. The eight non-EGARCH models are unaffected.

To facilitate exact reproduction, we provide a complete packaged environment as a
GitHub Release (see the repository's Releases page):

**`Reviewer_Packages.tar.xz`** (1.5 GB) — contains two independent approaches:

| Package | Size | Method | Platform |
|---|---|---|---|
| **Package A** (CondaPack) | ~492 MB | Pre-built binary environment with Intel MKL 2021.4.0, all scripts, data, ground-truth tables, and a pre-generated verification report | Windows |
| **Package B** (Docker) | ~989 MB | Docker OCI image with Intel MKL 2021.4.0, all scripts, data, and ground-truth tables | Cross-platform |

Both include `REVIEWER_GUIDE.md` with step-by-step instructions. Package A is the
recommended option for Windows users: simply extract the CondaPack tarball, activate
the environment, and run `verify_against_paper.py`. Package B is for cross-platform
use via Docker.

**Important:** Both packages use the native MKL instruction path. Do **not** set
`MKL_CBWR` — forcing a specific instruction set changes the floating-point path and
can push EGARCH into a different basin. The environments pre-configure
`MKL_NUM_THREADS=1` and `OMP_NUM_THREADS=1` for deterministic single-threaded
execution.
...
6. **pandas 2.x compatibility**: The original code (written 2022) requires two
   patches for pandas >= 2.0 (see `E:/code/generate_tables.py`).
